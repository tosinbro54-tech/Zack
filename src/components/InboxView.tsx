...
export const InboxView: React.FC<InboxViewProps> = ({ onAddToQueue, callGemini, voicePrompt }) => {
  ...
  const handleQueueReply = async () => {
    if (!replyText.trim() || !currentMsg) return;
    await api.post('/api/queue', { actionType: 'dm', targetLabel: currentMsg.name, text: replyText });
  ...
  return (
    <div>
      ...
      <div className="card-e mb-3 p-3 text-[12px] text-[var(--txt2)]">
        ⚠ Conversation list below is a preview — live LinkedIn inbox reading isn't built yet. Drafting and queuing a reply below IS real and sends to your approval queue.
      </div>
      ...
      <div className="grid md:grid-cols-[300px_1fr] grid-cols-1 border border-[var(--border)] rounded-[13px] overflow-hidden h-[520px]">
        <div className="border-right border-[var(--border)] flex flex-col h-full bg-[var(--sb)] overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {messages.map(m => (
              <div className={`inbox-item ${selectedId === m.id ? 'bg-[var(--bg3)]' : ''}`} onClick={() => handleSelectConv(m.id)} key={m.id}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-semibold flex items-center gap-1.5">
                    {m.name}
                    {m.unread && <span className="text-[9px] bg-[var(--pri)] text-white px-1.5 py-0.5 rounded">NEW</span>}
                  </div>
          ...
          {currentMsg ? (
            <>
              <div className="flex items-center gap-2.5 p-3.5 border-bottom border-[var(--border)]">
                <div className="avatar-sm w-9 h-9 text-xs font-semibold bg-[var(--bg3)]">{currentMsg.av}</div>
                <div>
              ...
              <div className="p-3.5 border-top border-[var(--border)] bg-[var(--bg2)]">
                <input className="inp mb-2 text-xs" placeholder="Instructions for AI (optional)…" value={instruction} onChange={e => setInstruction(e.target.value)} />
                <textarea className="ta text-xs mb-2 h-20" placeholder="Click Draft reply for AI, or type your own…" value={replyText} onChange={e => setReplyText(e.target.value)}></textarea>
                <div className="flex gap-2">
                  <button className="btn btn-out text-xs inline-flex items-center gap-1.5" onClick={handleDraftReply} disabled={drafting}>
                    {drafting ? <><span className="ai-spin"></span>Drafting...</> : '✨ Draft reply'}
                  </button>
                  <button className="btn btn-pri text-xs" onClick={handleQueueReply}>
                    → Queue for approval
                  </button>
                </div>
              </div>
